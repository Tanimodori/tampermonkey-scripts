...
监控组是 compose 里的 `stats` profile：Prometheus、Grafana 与三个 exporter。它要读 `stats/` 下的配置与面板文件，而部署 zip 里只有应用产物，所以这些文件随 compose 与 nginx 配置一起上传。

拉起、访问与排错见 [运行环境](setup.md)，每个指标的含义见 [指标](metrics.md)，面板的分组与表达式见 [Grafana 面板](grafana.md)。
