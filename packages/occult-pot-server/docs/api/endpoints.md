...
| POST | `/api/v1/pots` | 写入一个罐子（新增或更新） |
| GET  | `/healthz`     | 存活探针                   |
| GET  | `/readyz`      | 就绪探针                   |

没有「按 ID 取一个罐子」的端点：客户端拉整张表（这张表最多几十行），在本地按自己的需要查找。
...
- 服务端已按唯一行键（`区服|地图|ID`）去重，不会返回重复项：表里同键有多行时，只有 `最后一次进岛时间` 最大的那一行会出现在响应里。
...
写入是同步的：行先落到表里再回答 `200`，`data` 是写进去的那个罐子（只有五个字段，不含任何文档侧信息），`message` 是这条写入的确认信息。
...
这是一个 **upsert**，按唯一行键 `区服|地图|ID` 匹配：

- 表里已有同一个键的行：调用 `更新记录` 改写那一行，并删除同键的其余重复行（删除失败只记一条 warning，本次写入仍然算成功）；键上唯一的判断依据是这三个字段，与时间无关。
- 表里没有：走原来的 `添加记录` 追加一行。
- 因为要先看表里有没有，一次 POST 会先读一次整张表（可能多页），再发一次写入；没有重复行时不会有额外的删除调用。响应不区分「新增」与「更新」——两种情况下 `data` 都是那个罐子。
- 表拒绝写入时请求失败，不会写入任何内容，缓存也不会改变（重复行也不会被删）；失败的状态码见 [错误处理](errors.md)。没有队列、后台重试或可轮询的句柄。
- 写入成功后这条记录同时进入缓存，因此紧接着的列表读取能看到它，且不会回源。表与缓存都写入成功后才回答。
- 没有批次：一次请求写入一个罐子，行序等于请求到达顺序；同一个罐子重复上传得到的是同一行。
- 受 `writes` 限流。
...
nginx 只转发 `GET /api/v1/pots`、`POST /api/v1/pots` 与 `GET /readyz`；其余路径返回纯文本 `404 Not Found`，不带响应信封，也不转发给应用（已删除的 `GET /api/v1/pots/${potId}` 现在就走这条路）。nginx 另外有一道更松的限制（每 IP 20r/s，POST 2r/s），被它拒绝同样是 `429` 与 `ERR_RATE_LIMITED`，但没有 `RateLimit-*` 响应头，因此客户端应按 `code` 判断。nginx 的配置与核对方式见 [`deploy/README.md`](../../deploy/README.md)。
