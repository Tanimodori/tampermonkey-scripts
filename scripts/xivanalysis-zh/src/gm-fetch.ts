// 把 `xiv-api-provider` 要的 `FetchLike` 接口接到 `GM_xmlhttpRequest` 上。
//
// 为什么要它:`@grant none` 下脚本只能用宿主页的 fetch,跨源取 garland / 国服 xivapi 会被目标站的 CORS 拦掉
// (实测 garland `search.php` 无 `Access-Control-Allow-Origin`,报 CORS 阻断)。GM_xmlhttpRequest 走扩展代发,
...
export const gmFetch: FetchLike = (input, init) => {
  ...
  return new Promise<Response>((resolve, reject) => {
    ...
    const handle = GM_xmlhttpRequest({
      ...
      data: (init?.body ?? null) as GMXHROptions['data'],
      responseType: 'text',
      // 对齐 provider 的 `credentials: 'omit'`:不带目标域 Cookie。
      anonymous: true,
      onload: (res) => {
        settled = true;
        try {
          resolve(new Response(res.responseText, { status: res.status, statusText: res.statusText, headers: parseHeaders(res.responseHeaders) }));
        } catch (e) {
      ...
      onerror: (res) => {
        settled = true;
        reject(new TypeError(`GM network error: ${url} (status ${res?.status ?? 0})`));
      },
      ontimeout: () => {
        settled = true;
        reject(new DOMException(`GM timeout: ${url}`, 'TimeoutError'));
      },
      onabort: () => {
        if (settled) return;
        settled = true;
        reject(new DOMException(`GM aborted: ${url}`, 'AbortError'));
      },
    });
