 ...
 *
 * Measured against a live document, the answer is filed under `data.getSheet` and each entry carries
 * `isVisible` and `type` — the documentation's own example spells the visibility field `isVibile`, so
 * both spellings are kept here: one to answer with, one to prove the reader survives the other.
 */
