import { getSheetAnswer, sheet, sheetWithDocumentedSpelling } from '@test/testUtils/upstream/file.ts';
import { deleteRecordsAnswer, getRecordsAnswer, readRow, readRows, writtenRecordsAnswer, writtenRecordsWithoutId } from '@test/testUtils/upstream/record.ts';
import { refreshTokenAnswer, refreshTokenRefused, userInfoAnswer } from '@test/testUtils/upstream/token.ts';
 ...
 *
 * `test/testUtils/upstream/` is what the mocked upstream replies with, so if a sample and a schema
 * disagree, every test built on that sample is agreeing with a fiction. This is the one place that
 * catches the drift: a sample that stops parsing, or a schema that stops accepting what the live
 * document sends, fails here rather than making a green suite meaningless.
 */
...
describe('a page of records', () => {
  it('parses as GetRecordsResponse', () => {
    const answer = getRecordsAnswer({ records: readRows([{ recordID: 'r00001', values: { 区服: '鸟' } }]), total: 1, hasMore: false, next: 1 });
...
describe('the write answers', () => {
  it('parse as AddRecordsResponse and carry no timestamps', () => {
    const answer = writtenRecordsAnswer('addRecords', [{ recordID: 'rNew1', values: { 区服: '鸟' } }]);

    expect(AddRecordsResponseSchema.parse(answer).data.addRecords.records).toEqual([{ recordID: 'rNew1', values: { 区服: '鸟' } }]);
  });

  it('parse without a record id, the shape the service reports rather than fails on', () => {
    const answer = writtenRecordsAnswer('addRecords', writtenRecordsWithoutId([{ recordID: 'rNew1', values: { 区服: '鸟' } }]));
