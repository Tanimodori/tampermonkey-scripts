 ...
 * the configured window, and it is rebuilt when the configuration is replaced — plus the one thing
 * it deliberately does *not* do, which is retry a failing task (that is the transport's business).
 */
...
describe('throttle', () => {
  ...
  it('starts a fresh window when the configuration is replaced', async () => {
