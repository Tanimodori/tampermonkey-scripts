...
代价是每次构建都要读一遍缓存的 CSV 并哈希它(几十 KB 到数 MB,毫秒级);作为交换,解析与裁剪在模块命中时整个跳过。
...
生成的模块就是一份 `SheetRawData`:`{ origin, data }`,`data` 是整张网格——三行表头加数据行,列名照表原样,值全是字符串。这是 `xiv-api-provider/datamine` 的形状,不是这个包另造的一份。
