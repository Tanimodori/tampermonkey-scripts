...
} from '@/validation/upstream.ts';

/**
 * The wire contract itself, one describe per response type.
 *
 * Every example here is the shape the live test document actually answered with (measured 2026-09-19),
 * not the shape the documentation shows: the two differ, and the answers are what this service has to
 * read. What each case pins is the same pair of questions — does a real answer parse, and does something
 * that is *not* that answer fail? — because a schema that accepts too much hides a changed upstream, and
 * one that accepts too little fails a table read over a field nobody reads.
 */
    ...
    const answer = {
      ...
      data: {
        getRecords: {
          ...
          next: 2,
          total: 41,
          records: [{ recordID: 'rIQDa3', createTime: '1789289445000', creatorName: '', values: { 区服: [{ text: '鸟', type: 'text' }] } }],
        },
      },
    };

    const parsed = GetRecordsResponseSchema.parse(answer);
    expect(parsed.data.getRecords.records?.[0]?.recordID).toBe('rIQDa3');
    expect(parsed.data.getRecords.hasMore).toBe(true);
    // An answer is not stripped on the way in: what the operator quotes is what was sent.
...
describe('CommonRecord', () => {
  ...
  it('takes the two instants in whichever encoding the sheet used', () => {
    // Both arrived in the measured answer as strings; `epochMsOf` still accepts either.
...
describe('the write answers', () => {
  it('carry the id of the row they touched', () => {
    const answer = {
      ret: 0,
      msg: 'Succeed',
      data: { addRecords: { records: [{ recordID: 'r1G3ql', values: { ID: [{ text: '99-9-4000DEAD', type: 'text' }] } }] } },
    };
    expect(AddRecordsResponseSchema.parse(answer).data.addRecords.records?.[0]?.recordID).toBe('r1G3ql');
  });

  it('may carry no id at all, which the pot service reports rather than fails on', () => {
    // Measured, the document does answer with one; a document that answers without one is the case
    // `pot.spec.ts` pins, and the row type is what keeps it from being a crash.
...
describe('GetSheetResponse', () => {
  it('reads the sub-sheet list as the live document sends it', () => {
    const answer = { ret: 0, msg: 'Succeed', data: { getSheet: [{ isVisible: true, sheetID: 't00i2h', title: '智能表1', type: 'smartsheet' }] } };
    expect(GetSheetResponseSchema.parse(answer).data.getSheet[0]?.sheetID).toBe('t00i2h');
  });

  it('demands the id the store checks its configuration against', () => {
...
describe('the credential endpoints', () => {
  it('report the identity directly in `data`, with no section key', () => {
    ...
    const answer = { ret: 0, msg: 'Succeed', data: { openID: '90839a97', nick: 'tester', avatar: 'https://example.invalid/a.png', unionID: 'u1' } };
    expect(UserInfoResponseSchema.parse(answer).data.openID).toBe('90839a97');
  ...
  it('answer a refresh with the token fields, and refuse an envelope it never sends', () => {
    const answer = { access_token: 'fresh', token_type: 'Bearer', expires_in: 2_592_000, scope: 'scope.smartsheet', user_id: '90839a97' };
    expect(RefreshTokenResponseSchema.parse(answer).access_token).toBe('fresh');
    // A refusal is still an answer here: the store words it, and needs the body to do so.
...
describe('a row’s cell values', () => {
  it('are the columns the document happens to have, in whichever shape', () => {
    ...
    // Tolerant on purpose: a row the rules cannot make a pot out of is swept, not fatal to the read.
