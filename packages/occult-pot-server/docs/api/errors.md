...
| `PAYLOAD_TOO_LARGE`      | 413  | 请求体超过 `JSON_BODY_LIMIT`                                                             |
...
| `CONFIG_INVALID`         | 500  | 配置非法（启动即退出）、文档/子表 ID 解析失败，或凭据校验失败、Open-Id 与 token 不符     |
...
规则本身都在 `src/validation/`：`pot.ts`（罐子的规则与写入 body）、`sheet.ts`（表地址与行映射）、`config.ts`（`AppConfig`/`AppEnvConfig` 与其类型）、`utils.ts`（共享解析助手）。请求体与表里的行共用同一套规则，`Pot` 类型就是从 `potSchema` 推导出来的，所以模型和它的规则不会各自漂移。
...
| 类型                                           | 状态码 | 文案                                                  |
| ---------------------------------------------- | ------ | ----------------------------------------------------- |
| `entity.too.large`                             | 413    | `Request body exceeds the configured JSON_BODY_LIMIT` |
...
- `LOG_LEVEL` 的四档 `debug | info | warning | error` 就是 LogTape 的级别名，原样传给 `lowestLevel`；
...
- **启动期失败**：配置非法（列出全部问题后退出），或文档坐标解析/凭据校验失败（记 error 后拒绝启动）—— 见 `../README.md` 的快速开始。
- **凭据告警**：文档坐标解析成功时记一条 info（`Resolved the Tencent Docs document`）；凭据已过期或进入 `TOKEN_EXPIRY_WARN_MS` 时记一条 warning（`Access token has expired; …` / `Access token expires soon; schedule a credential rotation`）。
